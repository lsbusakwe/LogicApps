# Block IP via Conditional Access
...
[![Deploy to Azure Gov](https://aka.ms/deploytoazuregovbutton)](https://portal.azure.us/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Flsbusakwe%2FLogicApps%2Fmaster%2FBlockIPviaConditionalAccess%2Fazuredeploy.json)

## Overview
This Logic App automates blocking IP addresses via Azure AD Conditional Access by adding them to a named location (`MaliciousIPs`). It is triggered by an HTTP POST request containing `ipaddress` and `incident_id`, retrieves a client secret from Azure Key Vault, authenticates with Microsoft Graph API, updates the named location, and includes placeholder actions for notifications.

## Minimum Requirements
- **Azure Subscription**: With permissions to create Logic Apps, Key Vaults, and manage Azure AD app registrations.
- **Microsoft 365 E3 License**: Required for Azure AD Conditional Access.
...
The Logic App uses an Azure AD application to authenticate with Microsoft Graph API to manage Conditional Access named locations.

1. **Navigate to Azure AD**:
   - In the Azure portal, go to **Azure Active Directory** > **App registrations**.
   - Click **New registration**.
2. **Register the Application**:
   - **Name**: Enter a name (e.g., `BlockIPviaConditionalAccessApp`).
   - **Supported account types**: Select **Accounts in this organizational directory only**.
   - Click **Register**.
3. **Save App ID and Tenant ID**:
   - Copy the **Application (client) ID** (e.g., `76931a48-d8e8-44c3-bfde-4ebbbbb1ccab`). This is the `clientId` for the Logic App.
   - Copy the **Directory (tenant) ID** (e.g., `7fe872d7-cdb1-4bec-8f35-895e5cce3472`) from **Azure Active Directory** > **Overview**. This is the `tenantId`.
...
5. **Grant Admin Consent**:
   ...
   - Confirm the consent to allow the app to access Graph API without user interaction.

### 2. Identify the Azure Sentinel Workspace ID
The Logic App includes a `workspaceId` parameter for potential future integration with Azure Sentinel. If you plan to add Sentinel actions later, obtain the workspace ID.

1. **Navigate to Azure Sentinel**:
   - In the Azure portal, go to **Microsoft Sentinel**.
   - Select your workspace (e.g., `OpseraSentinel`).
2. **Copy the Workspace ID**:
   - In the workspace’s **Settings** > **Workspace settings**, find the **Workspace ID** (e.g., `ea2c0710-94a8-4647-aa9b-3ce2485bd016`).
   - Save this ID for use during deployment (optional, as it’s unused in the current workflow).

### 3. Create and Configure Azure Key Vault
The Logic App retrieves the client secret for Graph API authentication from a Key Vault secret named `BlockIPviaConditionalAccess`.

1. **Create a Key Vault**:
   - In the Azure portal, go to **Key vaults** > **Create**.
   - **Resource group**: Choose or create a resource group (e.g., `OpseraSentinel`).
   - **Key vault name**: Enter a unique name (e.g., `OpseraKeyVault`).
   - **Region**: Select the same region as your Logic App (e.g., `southafricanorth`).
...
3. **Add a Secret**:
   ...
   - **Name**: Enter `BlockIPviaConditionalAccess`.
   - **Value**: Paste the client secret for the Azure AD app registration:
     ...
     - Set a description (e.g., `BlockIPSecret`) and expiry (e.g., 1 year).
...
4. **Add Access Policy for the App Registration**:
   ...
   - **Select principal**: Search for the app registration by its name or Client ID (e.g., `BlockIPviaConditionalAccessApp`).
...
2. **Enter Parameters**:
   - **logicAppName**: Enter a unique name (e.g., `ConoscoSecurityLogicApp`).
   - **location**: Select the Azure region (e.g., `southafricanorth`).
   - **tenantId**: Enter the Tenant ID from the app registration (e.g., `7fe872d7-cdb1-4bec-8f35-895e5cce3472`).
   - **clientId**: Enter the Client ID from the app registration (e.g., `76931a48-d8e8-44c3-bfde-4ebbbbb1ccab`).
   - **workspaceId**: Enter the Sentinel Workspace ID (e.g., `ea2c0710-94a8-4647-aa9b-3ce2485bd016`) or use the default (optional).
3. **Deploy**:
...
1. **Configure the Key Vault Connection**:
   - In the Azure portal, navigate to the deployed Logic App (e.g., `ConoscoSecurityLogicApp`).
   ...
   - Click **Add new** to create a connection:
     ...
     - **Key Vault**: Choose the Key Vault created earlier (e.g., `OpseraKeyVault`).
   - Click **Create** and save the connection.
2. **Add Access Policy for the Logic App**:
   - In the Azure portal, go to the Key Vault (e.g., `OpseraKeyVault`).
   - Go to **Access policies** > **Create**.
   - **Permissions**: Select **Secret permissions** > **Get**.
   - **Select principal**: Search for the Logic App by its name (e.g., `ConoscoSecurityLogicApp`).
...
3. **Verify Execution**:
   ...
   - Check that the Logic App:
     - Retrieves the `BlockIPviaConditionalAccess` secret from Key Vault.
     - Authenticates with Microsoft Graph API.
     - Updates the `MaliciousIPs` named location in Azure AD Conditional Access.
...
### Troubleshooting
- **Deployment Errors**:
  - Ensure `logicAppName` is unique and `location` is valid (e.g., `southafricanorth`).
  - Check deployment logs in the Azure portal under **Resource group** > **Deployments**.
- **Key Vault Errors**:
  - Verify the Key Vault exists and contains the `BlockIPviaConditionalAccess` secret.
  - Ensure the Logic App’s Managed Identity and app registration have `Get` permissions in the Key Vault’s access policies.
- **Graph API Errors**:
  - Confirm the `clientId` and `tenantId` are correct and the app registration has `Policy.Read.All` and `Policy.ReadWrite.ConditionalAccess` permissions with admin consent.
