...
[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Flsbusakwe%2FLogicApps%2Fmaster%2FIsolateMachine%2Fazuredeploy.json)
[![Deploy to Azure Gov](https://aka.ms/deploytoazuregovbutton)](https://portal.azure.us/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Flsbusakwe%2FLogicApps%2Fmaster%2FIsolateMachine%2Fazuredeploy.json)

## Overview
This Logic App automates isolating a machine in Microsoft Defender for Endpoint as part of a security incident response. It is triggered by an HTTP POST request containing `machineid`, `machineName`, and `incident_id`, retrieves a client secret from Azure Key Vault, authenticates with the Defender for Endpoint API, isolates the specified machine, and includes placeholder actions for notifications.

## Minimum Requirements
- **Azure Subscription**: With permissions to create Logic Apps, Key Vaults, and manage Azure AD app registrations.
- **Microsoft 365 E5 or Microsoft Defender for Endpoint License**: Required for machine isolation via Defender for Endpoint API.
...
The Logic App uses an Azure AD application to authenticate with the Microsoft Defender for Endpoint API to isolate machines.

1. **Navigate to Azure AD**:
   - In the Azure portal, go to **Azure Active Directory** > **App registrations**.
   - Click **New registration**.
2. **Register the Application**:
   - **Name**: Enter a name (e.g., `IsolateMachineApp`).
...
5. **Grant Admin Consent**:
   ...
   - Confirm the consent to allow the app to isolate machines without user interaction.
...
The Logic App retrieves the client secret for Defender API authentication from a Key Vault secret named `MDR-IsolateMachine`.
...
3. **Add a Secret**:
   ...
   - **Value**: Paste the client secret for the Azure AD app registration:
     ...
     - Set a description (e.g., `IsolateMachineSecret`) and expiry (e.g., 1 year).
...
4. **Add Access Policy for the App Registration**:
   ...
   - **Select principal**: Search for the app registration by its name or Client ID (e.g., `IsolateMachineApp`).
...
2. **Enter Parameters**:
   - **logicAppName**: Enter a unique name (default: `IsolateMachine`).
...
1. **Configure the Key Vault Connection**:
   - In the Azure portal, navigate to the deployed Logic App (e.g., `IsolateMachine`).
...
2. **Add Access Policy for the Logic App**:
   ...
   - **Permissions**: Select **Secret permissions** > **Get**.
   - **Select principal**: Search for the Logic App by its name (e.g., `IsolateMachine`).
...
3. **Verify Execution**:
   ...
   - Check that the Logic App:
     - Retrieves the `MDR-IsolateMachine` secret from Key Vault.
     - Authenticates with the Microsoft Defender for Endpoint API.
     - Isolates the specified machine.
...
- **Key Vault Errors**:
  - Verify the Key Vault exists and contains the `MDR-IsolateMachine` secret.
  - Ensure the Logic App’s Managed Identity and app registration have `Get` permissions in the Key Vault’s access policies.
- **Defender API Errors**:
  - Confirm the `clientId` and `tenantId` are correct and the app registration has `Machine.Isolate` permission with admin consent.
